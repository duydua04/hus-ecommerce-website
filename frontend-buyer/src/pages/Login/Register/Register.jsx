...
function BuyerRegister() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      // Chuyển về trang login sau 2s đăng ký thành công
      setTimeout(() => {
      ...
      if (message.includes("400")) {
         ...
         setErrorMessage("Dữ liệu không hợp lệ (Kiểm tra email/password)");
      } else {
  ...
  const handleBackToLogin = () => navigate("/buyer/login");
        ...
        <div className="register-form-section">
          ...
          <form onSubmit={handleRegister}>
            <div className="form-group">
              ...
              <input id="email" name="email" type="email" placeholder="Nhập email..." value={formData.email} onChange={handleChange} autoComplete="email" disabled={loading} />
            </div>

            <div className="form-group">
              <label htmlFor="phone">Số điện thoại <span className="required">*</span></label>
              <input id="phone" name="phone" type="tel" placeholder="Nhập số điện thoại..." value={formData.phone} onChange={(e) => { const value = e.target.value.replace(/\D/g, ""); setFormData((prev) => ({ ...prev, phone: value })); }} maxLength="11" autoComplete="tel" disabled={loading} />
            </div>

            <div className="form-row">
              <div className="form-group">
                <label htmlFor="fname">Họ <span className="required">*</span></label>
                <input id="fname" name="fname" type="text" placeholder="Họ..." value={formData.fname} onChange={handleChange} autoComplete="given-name" disabled={loading} />
              </div>
              <div className="form-group">
                <label htmlFor="lname">Tên <span className="required">*</span></label>
                <input id="lname" name="lname" type="text" placeholder="Tên..." value={formData.lname} onChange={handleChange} autoComplete="family-name" disabled={loading} />
              </div>
            </div>
            ...
            <div className="form-group">
              ...
              <div className="input-wrapper">
                <input id="password" name="password" type={showPassword ? "text" : "password"} placeholder="Nhập mật khẩu (tối thiểu 6 ký tự)..." value={formData.password} onChange={handleChange} autoComplete="new-password" disabled={loading} />
            ...
            <div className="form-group">
              ...
              <div className="input-wrapper">
                <input id="confirmPassword" name="confirmPassword" type={showConfirmPassword ? "text" : "password"} placeholder="Nhập lại mật khẩu..." value={formData.confirmPassword} onChange={handleChange} onKeyDown={(e) => e.key === "Enter" && handleRegister(e)} autoComplete="new-password" disabled={loading} />
            ...
            <div className="checkbox-group">
              <input id="terms" type="checkbox" checked={agreedToTerms} onChange={(e) => setAgreedToTerms(e.target.checked)} disabled={loading} />
            ...
            <button type="submit" className={`register-btn ${loading ? "loading" : ""}`} disabled={loading}>
              {loading ? <><span className="spinner"></span> Đang xử lý...</> : "Đăng ký"}
            </button>
          </form>
          ...
          <div className="footer-links">
            ...
            <button type="button" className="link-btn" onClick={handleBackToLogin}>Đăng nhập ngay</button>
          </div>
        </div>
      </div>
    </div>
