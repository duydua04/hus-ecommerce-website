...
from ...schemas.common import Page, PageMeta, OrderStatus, PaymentStatus
...
def list_orders_for_seller(
    ...
    if order_status:
        qset = qset.filter(Order.order_status == str(order_status))
    if payment_status:
        qset = qset.filter(Order.payment_status == str(payment_status))
    if date_from:
    ...
    if q:
        # nối với tên buyer để tìm theo tên; cũng cho phép match exact order_id
        buyer_sub = (
            db.query(
        ...
        key = q.strip()
        qset = qset.outerjoin(buyer_sub, buyer_sub.c.oid == Order.order_id).filter(
            or_(
    ...
    total = qset.count()
    rows = (
        qset.order_by(Order.order_date.desc())
...
# ----------------------- State transitions (thủ công) -----------------------

def _require_status(current: str, allowed: List[str]):
    if current not in allowed:
        _bad(f"Invalid state transition from '{current}'")

def confirm_order(db: Session, seller_id: int, order_id: int):
    """
    Thủ công: chỉ đổi pending -> processing, KHÔNG kiểm tra payment_method/payment_status.
    """
    ensure_order_belongs_to_seller(db, seller_id, order_id)
    o = db.query(Order).filter(Order.order_id == order_id).with_for_update().first()
    if not o:
        _404()
    _require_status(str(o.order_status), ["pending"])
...
def mark_shipped(db: Session, seller_id: int, order_id: int):
    ...
    if not o:
        _404()
    _require_status(str(o.order_status), ["processing"])
