...
class SellerReviewService(BaseReviewService):
    ...
    async def list_my_reviews(
        ...
        # Lọc theo tên SP (Tìm ID bên Postgres -> Filter Mongo)
        if product_name:
            p_ids = [p.product_id for p in self.db.query(Product.product_id).filter(
...
def get_seller_review_service(db: Session = Depends(get_db)):
